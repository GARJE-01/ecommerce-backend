...
exports.sendMail = async function ({to, subject, text, html}) {
    let info = await transporter.sendMail({
        from: '"E-commerce" <mayurgarje27@gmail.com>', // sender address
        to,
        subject,
        text,
        html
      ...
      <tr>
        <td align="center" bgcolor="#D2C7BA">
          <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px;">
            ...
            <tr>
              <td align="left" bgcolor="#ffffff" style="padding: 24px; font-family: 'Source Sans Pro', Helvetica, Arial, sans-serif; font-size: 16px; line-height: 24px; border-bottom: 3px solid #D2C7BA">
...
// Set up nodemailer transporter with credentials stored in .env
let transporter = nodemailer.createTransport({
  host: "smtp.gmail.com",
  port: 587,
  secure: false, // true for 465, false for other ports
  auth: {
    user: process.env.MAIL_USER, // Email address stored in .env
    pass: process.env.MAIL_PASSWORD, // Email password stored in .env
  },
});
...
// Sanitize the user object before sending it
exports.sanitizeUser = (user) => {
  return { id: user.id, role: user.role };
};
...
exports.sendMail = async function ({ to, subject, text, html }) {
  let info = await transporter.sendMail({
    from: '"E-commerce" <' + process.env.MAIL_USER + '>', // sender address from .env
    to,
    subject,
    text,
    html
...
// Function to generate email invoice template
exports.invoiceTemplate = function(order) {
  return (`<!DOCTYPE html>
<html>
<head>
  <!-- The rest of the email template -->
</head>
<body style="background-color: #D2C7BA;">
  <!-- start body -->
  <table border="0" cellpadding="0" cellspacing="0" width="100%">
    ...
    <tr>
      <td align="center" bgcolor="#D2C7BA">
        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width: 600px;">
          <!-- Order ID -->
          <tr>
            <td align="left" bgcolor="#D2C7BA" width="60%" style="padding: 12px;">
              <strong>Order #</strong>
            </td>
            <td align="left" bgcolor="#D2C7BA" width="20%"></td>
          ...
          ${order.items.map(item => `
          <tr>
            <td align="left" width="60%" style="padding: 6px 12px;">${item.product.title}</td>
            <td align="left" width="20%" style="padding: 6px 12px;">${item.quantity}</td>
            <td align="left" width="20%" style="padding: 6px 12px;">$${Math.round(item.product.price * (1 - item.product.discountPercentage / 100), 2)}</td>
          </tr>`).join('')}
          <!-- Order total -->
          <tr>
            <td align="left" width="60%" style="padding: 12px; border-top: 2px dashed;">
              <strong>Total</strong>
            </td>
            <td align="left" width="20%" style="padding: 12px; border-top: 2px dashed;">
